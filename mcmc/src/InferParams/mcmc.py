...
class Comp:
    ...
    def __init__(self,psd = 1e-4,rsd=1e-3,alsd = 0.01,
        ...
        self.data_temp = defaultdict(dict)
        self.baseroot = ()
        self.al = {}
        self.mu = 0.2
        self.p0 = 1e-5
    ...
    def get_data(self,f,spec):
        ...
        for line in fh:
            ...
            if self.skipnanDam and (locdata[7] == "NA" or np.isnan(float(locdata[7]))):
                continue
    ...
    def rep_NaN_mean(self,spec):
        ...
        for lab in self.data[spec]: 
            if lab in self.data_temp[spec] and lab != self.sc:
                self.data[spec][lab] = self.data_temp[spec][lab][:]
    ...
    def rep_NaN_random(self,spec):
        ...
        for lab in self.data[spec]: 
            ...
            if nans == len(self.data[spec][lab]):
                ...
                continue
            nans = 0
            self.data_temp[spec][lab] = self.data[spec][lab][:]
            for k in range(len(self.data[spec][lab])):
                if np.isnan(float(self.data[spec][lab][k])):
    ...
    def get_r(self,spec):
        ...
        for i in range(len(D0)):
            # uncomment next line, comment line after, and recompile to use BH model
    ...
    def sim_seed(self,spec):
        ...
        while 1:
            ...
            if br == 0:
                break
        mean_sc = (1-self.p0)*self.r0/(self.p0)
        mean_sc_sim = 0
        r_loc = self.get_r(spec)
        totnum = len(r_loc)
        
        for i in xrange(len(r_loc)):
            mean_sc_sim +=  r_loc[i]*(1-self.p0)/self.p0
        mean_sc_sim /= totnum 
        fac = mean_sc/mean_sc_sim
        self.r0 *= fac
    ...
    def return_max_ll(self,spec,writefile):
        ...
        BIC1 = np.log(len(self.data[spec][self.sc])+0.)*8 - 2*ll1
        AIC1 = 2*8 - 2*ll1
        
        BIC0 = np.log(len(self.data[spec][self.sc])+0.)*2 - 2*ll0
        AIC0 = 2*2 - 2*ll0

        wh = open(writefile,'w')

        wh.write(str(ll1)+' '+str(ll0)+' '+str(AIC1)+' '+str(AIC0)+' '+str(BIC1)+' '+str(BIC0)+'\n')
