...
class Comp:
    ...
    def modelrvs(self,spec): 
        ...
        for i in range(len(self.data[spec][self.sc])):
            if np.random.random() > self.q0:
                self.data[spec][self.sc][i] =  nbinom.rvs(n=self.r_loc[i],p=self.p0)
            else:
    ...
    def compute_pred_split(self,spec):
        ...
        for ind in range(len(self.pred_data[spec][tags[0]])):
            denom = 1.
            for i in range(6):
                if self.data[spec][self.sc][i] == 0:
                    denom+=(1-self.q)*self.pred_data[spec][tags[i]][ind]*params[i]/self.rescale_rat[i]
                else:
